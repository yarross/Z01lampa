...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  // === BACKDOOR TEST: TV-логгер на экран ===
  window.z01_logs = [];
  window.z01_tvlog = function(msg, time) {
    window.z01_logs = window.z01_logs || [];
    window.z01_logs.push(msg);
    console.log('[Z01 TV]', msg);
    try { Lampa.Noty.show('[Z01] ' + msg, time || 8000); } catch(e) {}
  };
  window.z01_showReport = function(title, lines) {
    var html = '<div style="font-size:1.15em;line-height:1.6;padding:0.5em;">';
    lines.forEach(function(l) {
      var color = l.indexOf('❌') >= 0 ? '#ff5555' : (l.indexOf('✅') >= 0 ? '#55ff55' : '#fff');
      html += '<div style="margin-bottom:0.4em;color:' + color + ';">' + l + '</div>';
    });
    html += '</div>';
    Lampa.Modal.open({
      title: title || 'Z01 Security Test',
      html: html,
      onBack: function() {
        Lampa.Modal.close();
  ...
  // === /BACKDOOR TEST ===
    ...
    function zpremTrial(callback, forceEmail, forceUid) {
      // === BACKDOOR TEST: ПОЛНАЯ ПОДМЕНА + TV-ЛОГ ===
      window.z01_logs = [];
      z01_tvlog('🔴 НАЧАЛО ТЕСТА БЕЗОПАСНОСТИ', 10000);

      // Состояние ДО
      var oldEmail = Lampa.Storage.get('account_email', '') || 'НЕТ';
      var oldUid = Lampa.Storage.get('lampac_unic_id', '') || 'НЕТ';
      var oldUsed = Lampa.Storage.get('zprem_trial_used', '') ? 'ДА' : 'НЕТ';
      z01_tvlog('Было: Email=' + oldEmail.substring(0,14) + '... UID=' + oldUid.substring(0,10) + '... Used=' + oldUsed, 10000);
      ...
      z01_tvlog('Стало: Email=' + fakeEmail + ' UID=' + fakeUid.substring(0,16) + '...', 10000);

      var email = fakeEmail;
      var uid = fakeUid;
      if (!email || !uid) { 
        z01_tvlog('❌ Ошибка: email/uid пустые', 10000);
        if (callback) callback(false, 'no_data'); 
        return; 
      }

      var trialUrl = ZPREM_TRIAL_URL + '?email=' + encodeURIComponent(email) + '&uid=' + encodeURIComponent(uid);
      z01_tvlog('➡️ Отправляем запрос...', 6000);
      ...
      net.silent(trialUrl, function(resp) {
        ...
        z01_tvlog('⬅️ Ответ получен за ' + elapsed + 'мс', 6000);

        try {
          if (typeof resp === 'string') resp = JSON.parse(resp);
          var status = resp.status || 'unknown';
          var hasKey = resp.zpremkey ? 'ЕСТЬ' : 'НЕТ';

          if (resp.status === 'activated' && resp.zpremkey) {
            z01_tvlog('✅ СЕРВЕР ВЫДАЛ ТРИАЛ!', 10000);
            Lampa.Storage.set('zpremkey', resp.zpremkey);
            Lampa.Storage.set('zprem_expires', resp.expires_at);
            ...
            setTimeout(function() {
              z01_showReport('🔴 ДЫРА НАЙДЕНА!', [
                '✅ Триал выдан на НОВЫЙ email!',
                'Email: ' + fakeEmail,
                'UID: ' + fakeUid.substring(0,20) + '...',
                'Срок: ' + (resp.expires_at || 'неизвестно'),
                '',
                '❌ Защита ТОЛЬКО по email.',
                '❌ Любой может менять почту в настройках',
                '❌ и получать триал бесконечно.',
                '',
                '🔧 Хотфикс: в trial.php добавить',
                'проверку по IP-адресу/подсети.'
              ]);
            }, 1200);

            if (callback) callback(true, 'activated');
          } else {
            z01_tvlog('❌ СЕРВЕР ОТКАЗАЛ: ' + status, 10000);

            setTimeout(function() {
              if (status === 'already_used') {
                z01_showReport('🟢 ЗАЩИТА РАБОТАЕТ!', [
                  '❌ Триал НЕ выдан.',
                  'Причина: ' + status,
                  '',
                  'Email: ' + fakeEmail,
                  'UID: ' + fakeUid.substring(0,20) + '...',
                  '',
                  '✅ Сервер отказал даже на',
                  '✅ новый email + новый uid.',
                  '',
                  'Значит есть защита по:',
                  '• IP-адресу / подсети',
                  '• Или другому fingerprint.',
                  '',
                  '🟢 Всё безопасно, разраб молодец!'
                ]);
              } else {
                z01_showReport('🟡 РЕЗУЛЬТАТ ТЕСТА', [
                  'Статус: ' + status,
                  'Email: ' + fakeEmail,
                  'UID: ' + fakeUid.substring(0,20) + '...',
                  '',
                  'Неизвестный ответ сервера.',
                  'Покажи это разработчику.'
                ]);
              }
            }, 1200);
          ...
          z01_tvlog('❌ Ошибка парсинга: ' + e.message, 10000);
          if (callback) callback(false, 'parse_error'); 
        }
      }, function(er) {
        z01_tvlog('❌ СЕТЕВАЯ ОШИБКА', 10000);
    ...
    Lampa.SettingsApi.addParam({
      ...
      onRender: function(item) {
        item.css('display', '');
        var curEmail = Lampa.Storage.get('account_email', '') || 'НЕТ';
        var curUid = Lampa.Storage.get('lampac_unic_id', '') || 'НЕТ';
        item.find('.settings-param__name').text('🎁 [SECURITY TEST] Проверить защиту триала');
        item.find('.settings-param__descr,.settings-param__status').text('Email: ' + curEmail.substring(0,12) + '... UID: ' + curUid.substring(0,8) + '... | Нажми для теста');
      },
      onChange: function() {
        z01_tvlog('🚀 Запускаем полный тест...', 6000);
        zpremTrial(function(ok, reason) {
          if (ok) {
            Lampa.Noty.show('✅ ТРИАЛ ВЫДАН! Смотри отчёт на экране!', 10000);
          } else if (reason === 'already_used') {
            Lampa.Noty.show('❌ СЕРВЕР ОТКАЗАЛ: already_used. Смотри отчёт!', 10000);
          } else {
            Lampa.Noty.show('❌ Результат: ' + reason, 8000);
          }
        });
      }
    });
    ...
    // === BACKDOOR TEST: Ручной тестер ===
    window.z01_testTrial = function(customEmail, customUid) {
      zpremTrial(function(ok, reason){
        // результат уже покажет Modal
      }, customEmail, customUid);
    };
    // === /BACKDOOR TEST ===
