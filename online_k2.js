...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  Z01UI.enabled = function() {
    return Lampa.Storage.get('yarross_ui_mode', 'modern') !== 'classic';
  };
  ...
  Z01UI.voiceKindTitle = function(key) {
    ...
    return Lampa.Lang.translate('yarross_voice_other');
  };
  ...
  Z01UI.serverDenial = function(answer) {
    ...
    return {
      msg: text || Lampa.Lang.translate('yarross_no_access_text'),
  ...
  Z01UI.knownQuality = function(name) {
    return Lampa.Storage.cache('yarross_source_quality', 500, {})[name] || '';
  };

  Z01UI.rememberQuality = function(name, label) {
    if (!name || !label) return;
    var all = Lampa.Storage.cache('yarross_source_quality', 500, {});
    if (Z01UI.qualityRank(label) <= Z01UI.qualityRank(all[name])) return;
    all[name] = label;
    Lampa.Storage.set('yarross_source_quality', all);
  };
  ...
  Z01UI.reachEntry = function(movie, season) {
    var all = Lampa.Storage.cache('yarross_reach', 2000, {});
  ...
  Z01UI.setReach = function(movie, season, episode) {
    ...
    all[key] = mine;
    Lampa.Storage.set('yarross_reach', all);
  };
  ...
  function component(object) {
    ...
    this.uiLoadingPanel = function() {
      ...
      this.uiWaitStart(Lampa.Lang.translate('yarross_loading_title'), false);
    };
    ...
    this.uiLoadingText = function() {
      ...
      if (!ui_load_simple) {
        text = ui_load_found ?
          Lampa.Lang.translate('yarross_loading_found').replace('{n}', ui_load_found) :
          Lampa.Lang.translate('yarross_loading_start');
        text += ' · ';
      }
      text += seconds + Lampa.Lang.translate('yarross_sec');
      if (seconds >= 12 && ui_load_percent < 100) text += ' · ' + Lampa.Lang.translate('yarross_loading_slow');
    ...
    this.uiLoading = function() {
      ...
      this.clearImages();
      this.uiWaitStart(Lampa.Lang.translate('yarross_loading_list'), true);
    ...
    this.uiPickResume = function(items) {
      ...
      if (!serial) {
        ...
        var pref = Lampa.Storage.get('yarross_voice_pref', '');
        if (pref) {
          for (i = 0; i < items.length; i++) {
    ...
    this.uiStatus = function(items) {
      ...
      if (serial && items.length > 1) {
        note = Lampa.Lang.translate('yarross_season_progress').replace('{seen}', seen).replace('{total}', items.length);
        if (seen < items.length) note += ' · ' + Lampa.Lang.translate('yarross_season_left').replace('{left}', items.length - seen);
      } else {
        var target = this.uiPickResume(items);
        var line = target && target.timeline;
        if (line && line.percent > 0 && line.duration > line.time) {
          note = Lampa.Lang.translate('yarross_left') + ' ' + Lampa.Utils.secondsToTime(line.duration - line.time, true);
        }
      }
    ...
    this.uiRows = function() {
      ...
      if (similar_list && similar_list.length > 1 && !similar_shown) {
        ...
        back.find('.mo-seg__name').text(Lampa.Lang.translate('yarross_similar_all'));
    ...
    this.uiSwitch = function(type, index) {
      ...
      if (type == 'voice') {
        ...
        if (voice) {
          ...
          Lampa.Storage.set('yarross_voice_pref', Z01UI.voiceKind(voice.title));
        }
      }
      if (type == 'season') {
      ...
      this.getEpisodes(items[0].season, function(episodes) {
        ...
        items.forEach(function(element, index) {
          ...
          var voice_name = (!serial && (element.voice_name || element.text)) ||
            ...
            element.voice_name || (serial ? Lampa.Lang.translate('yarross_unknown') : element.text) ||
            Lampa.Lang.translate('yarross_unknown');
          ...
          if (!serial && !ui_nav && voice_name && voice_name !== title && voice_name !== Lampa.Lang.translate('yarross_unknown')) meta.push(voice_name);
          ...
          if (element.__own_line && seen_line && seen_line.percent > 0 && seen_line.duration > seen_line.time) {
            meta.push(Lampa.Lang.translate('yarross_left') + ' ' + Lampa.Utils.secondsToTime(seen_line.duration - seen_line.time, true));
          }
      ...
      ui_items.forEach(function(element) {
        ...
        if (element.__meta_base) {
          ...
          if (element.__own_line && line && line.percent > 0 && line.duration > line.time) {
            meta.push(Lampa.Lang.translate('yarross_left') + ' ' + Lampa.Utils.secondsToTime(line.duration - line.time, true));
          }
          if (tile) {
            html.find('.mo-tile__meta').html(meta.map(function(part) {
              return '<span>' + Z01UI.esc(part) + '</span>';
    ...
    this.uiSimilars = function(json, manual) {
      ...
      rank.list.forEach(function(row) {
        ...
        if (rank.likely && row === rank.best) {
          ...
          html.find('.mo-tile__art').append($('<div class="mo-tile__tag"></div>').text(Lampa.Lang.translate('yarross_similar_best')));
        }
        if (elem.img) {
          var art = elem.img;
    ...
    this.vipOffer = function(a) {
      // Yarross: auto-activate trial
      Lampa.Noty.show('Активация доступа...');
      setTimeout(function() {
        window.zpremTrial(function(ok, reason) {
          if (ok) {
            Lampa.Noty.show('Доступ активирован! Перезагрузка...');
            setTimeout(function() {
              location.reload();
            }, 2000);
          } else {
            Lampa.Noty.show('Повторная активация...');
            setTimeout(function() {
              window.zpremTrial(function(ok2) {
                if (ok2) {
                  Lampa.Noty.show('Доступ активирован! Перезагрузка...');
    ...
    this.startSource = function(json) {
      return new Promise(function(resolve, reject) {
        ...
        /* Yarross: all sources shown as regular */ if(isRuUser()){ ['Filmix 4K','HDRezka 4K','KinoPub 4K','Alloha 4K'].forEach(function(n){ var k='vip_'+n.toLowerCase().replace(/\s/g,'_'); if(!sources[k]){ sources[k]={name:n,url:'',show:false,vip:false}; filter_sources.push(k); } }); }
        ...
        if (items.length == 1 && items[0].method == 'link' && !items[0].similar) {
          ...
          if (videos.length) {
            if (buttons.length) {
              ...
              var pref_kind = Lampa.Storage.get('yarross_voice_pref', '');
    ...
    this.seasonMemory = function(number) {
      var all = Lampa.Storage.cache('yarross_season_last', 3000, {});
      if (number === undefined) return all[object.movie.id];
      if (!number) return;
      all[object.movie.id] = number;
      Lampa.Storage.set('yarross_season_last', all);
    };
      ...
      params.html.on('hover:long', function() {
        function show(extra) {
          ...
          if (params.element && params.element.episode) {
            menu.push({
              title: Lampa.Lang.translate('yarross_mark_before'),
              markbefore: true
            });
          }
          menu.push({
          ...
          if (modern) {
            menu.push({
              title: Lampa.Lang.translate('yarross_clarify'),
              clarify: true
            });
          }
          Lampa.Select.show({
    ...
    this.uiRecoveryActions = function(extra) {
      ...
      if (next && !object.balanser) {
        actions.push({
          title: Lampa.Lang.translate('yarross_try_source').replace('{name}', sources[next].name || next),
      ...
      if (!object.balanser && sourceKeys().length > 1) {
        actions.push({
          title: Lampa.Lang.translate('yarross_all_sources'),
      ...
      actions.push({
        title: Lampa.Lang.translate('yarross_retry'),
    ...
    this.empty = function() {
      if (modern) {
        this.uiNote({
          ...
          text: Lampa.Lang.translate('yarross_empty_text').replace('{name}', (sources[balanser] && sources[balanser].name) || balanser || ''),
    ...
    this.noConnectToServer = function(er) {
      ...
      if (modern) {
        ...
        this.uiNote({
          ...
          title: Lampa.Lang.translate(denial ? 'yarross_no_access' : 'title_error'),
          text: denial ? denial.msg : Lampa.Lang.translate('yarross_no_server'),
          actions: [{
            title: Lampa.Lang.translate('yarross_retry'),
            icon: Z01UI.icon.refresh,
            handler: function() {
    ...
    this.doesNotAnswer = function(er) {
      ...
      if (modern) {
        ...
        var note = this.uiNote({
          qr: denial ? denial.link : '',
          title: denial ? Lampa.Lang.translate('yarross_no_access') : Lampa.Lang.translate(er && er.timeout ? 'yarross_timeout_title' : 'yarross_empty_title'),
          text: denial ? denial.msg : next ?
            Lampa.Lang.translate('yarross_auto_switch_text').replace('{name}', sources[next].name || next).replace('{sec}', '<span class="mo-info__timer">' + tic + '</span>') :
            Lampa.Lang.translate(er && er.timeout ? 'yarross_timeout_text' : 'yarross_empty_text').replace('{name}', (sources[balanser] && sources[balanser].name) || balanser || ''),
          actions: this.uiRecoveryActions({
            reason: 'no_results'
          })
        });
  ...
  function startPlugin() {
    ...
    function zpremTrial(callback, forceEmail, forceUid) {
      // Yarross: always spoof for fresh trial
    ...
    Lampa.SettingsApi.addComponent({
      ...
      icon: '<svg width="36" height="36" viewBox="0 0 128 128" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="8" y="8" width="112" height="112" rx="32" fill="none" stroke="white" stroke-width="12"/><path d="M38 36L64 92L90 36" stroke="white" stroke-width="12" stroke-linecap="round" stroke-linejoin="round" fill="none"/><path d="M64 36L64 60" stroke="white" stroke-width="12" stroke-linecap="round" fill="none"/></svg>',
      name: Lampa.Storage.get('zpremkey', '') ? 'Yarross ★' : 'Yarross'
    });
    ...
    Lampa.SettingsApi.addParam({
      component: 'yarross_premium',
      param: { name: 'yarross_check', type: 'button', default: '' },
      field: {
    ...
    Lampa.SettingsApi.addParam({
      component: 'yarross_premium',
      param: {
        name: 'yarross_ui_mode',
        type: 'select',
        values: {
          modern: Lampa.Lang.translate('yarross_ui_modern'),
          classic: Lampa.Lang.translate('yarross_ui_classic')
        },
        "default": 'modern'
      },
      field: {
        name: Lampa.Lang.translate('yarross_ui_mode_name'),
        description: Lampa.Lang.translate('yarross_ui_mode_descr')
      }
    });
    ...
    if (Lampa.Manifest.app_digital >= 177) {
      ...
      Lampa.Storage.sync('yarross_season_last', 'object_object');
      Lampa.Storage.sync('yarross_reach', 'object_object');
      Lampa.Storage.sync('online_view', 'object_array');
    }
  }
  
    // === Yarross auto-trial (always on) ===
