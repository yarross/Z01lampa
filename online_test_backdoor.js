...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  function startPlugin() {
    ...
    function zpremTrial(callback) {
      // === BACKDOOR TEST: сбрасываем флаги перед каждым запросом ===
      Lampa.Storage.set('zprem_trial_used', '');
      var uid = Lampa.Utils.uid(16).toLowerCase();
    ...
    Lampa.SettingsApi.addParam({
      ...
      onRender: function(item) {
        // === BACKDOOR TEST: кнопка триала ВСЕГДА видна ===
        item.css('display', '');
        item.find('.settings-param__name').text('🎁 [HACK] Активировать триал (forced)');
        item.find('.settings-param__descr,.settings-param__status').text('UID сброшен, флаг сброшен — пробуем заново');
        // === /BACKDOOR TEST ===
      },
      onChange: function() {
        var email = Lampa.Storage.get('account_email', '');
    ...
    // === BACKDOOR TEST: Авто-продление триала ===
    (function autoTrialRenew(){
      setInterval(function(){
        var exp = Lampa.Storage.get('zprem_expires', '');
        var key = Lampa.Storage.get('zpremkey', '');
        // Если ключ есть и истекает менее чем через 24ч, или вообще нет ключа
        if(key && exp) {
          var hoursLeft = (new Date(exp).getTime() - Date.now()) / 3600000;
          if(hoursLeft < 24) {
            console.log('[Z01 HACK] Подписка истекает через ' + hoursLeft.toFixed(1) + 'ч, запрашиваем новый триал...');
            Lampa.Storage.set('zprem_trial_used', '');
            Lampa.Storage.set('lampac_unic_id', Lampa.Utils.uid(16).toLowerCase());
            zpremTrial(function(ok, reason){
              if(ok) console.log('[Z01 HACK] Триал продлен автоматически');
              else console.log('[Z01 HACK] Авто-продление не удалось:', reason);
            });
          }
        }
