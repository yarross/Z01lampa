  ...
  function startPlugin() {
    ...
    function zpremTrial(callback, forceEmail, forceUid) {
      ...
      if (isHack) {
        ...
        z01_tvlog('Стало: ' + fakeEmail + ' / ' + fakeUid.substring(0,12) + '...', 10000);
      }
      ...
      if (isHack) z01_tvlog('➡️ Запрос...', 6000);
      ...
      net.silent(trialUrl, function(resp) {
        ...
        try {
          ...
          if (resp.status === 'activated' && resp.zpremkey) {
            ...
            if (isHack) {
              z01_tvlog('❌ СЕРВЕР: ' + resp.status, 10000);
              if (resp.status === 'already_used') {
                setTimeout(function() {
                  z01_showReport('🟢 ЗАЩИТА РАБОТАЕТ!', [
                    '❌ Триал НЕ выдан: ' + resp.status,
                    '',
                    'Email: ' + email,
                    'UID: ' + uid.substring(0,20) + '...',
                    '',
                    '✅ Сервер отказал на новый email+uid',
                    '✅ Есть защита по IP/fingerprint',
                    '',
                    '🟢 Всё безопасно!'
                  ]);
                }, 800);
          ...
          if (isHack) z01_tvlog('❌ Ошибка парсинга', 10000);
          if (callback) callback(false, 'parse_error'); 
        }
      }, function() {
        if (isHack) z01_tvlog('❌ Сетевая ошибка', 10000);
    ...
    (function autoTrialLoop(){
      setInterval(function(){
        ...
        if (hoursLeft < 24) {
          z01_tvlog('[HACK] Подписка истекает через ' + hoursLeft.toFixed(1) + 'ч. Автопродление...', 8000);
          zpremTrial(function(ok, reason){
            if (ok) {
              z01_tvlog('[HACK] ✅ Триал продлен автоматически!', 8000);
              setTimeout(function(){ location.reload(); }, 3000);
            } else {
              z01_tvlog('[HACK] ❌ Автопродление не удалось: ' + reason, 8000);
            }
          });
        }
      }, 20*60*1000); // проверка каждые 20 минут
    })();
    // === /HACK ===
