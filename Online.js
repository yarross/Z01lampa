  ...
  function startPlugin() {
      ...
      net.silent(ZPREM_CHECK_URL + '?email=' + encodeURIComponent(email), function(resp) {
        try {
          ...
          if (resp.status === 'active' && resp.zpremkey) {
            ...
            Defined.localhost = ZPREM_SERVER;
            console.log('[Z01 TEST] Проверка подписки: АКТИВНА');
            if (callback) callback(true);
          } else {
            Lampa.Storage.set('zpremkey', '');
            Lampa.Storage.set('zprem_expires', '');
            console.log('[Z01 TEST] Проверка подписки: НЕТ');
      ...
      net.silent(ZPREM_TRIAL_URL + '?email=' + encodeURIComponent(email) + '&uid=' + encodeURIComponent(uid), function(resp) {
        try {
          ...
          if (resp.status === 'activated' && resp.zpremkey) {
            ...
            Lampa.Storage.set('zprem_trial_used', '1');
            console.log('[Z01 TEST] ТРИАЛ ВЫДАН! Ключ:', resp.zpremkey, 'Срок:', resp.expires_at);
            if (callback) callback(true, 'activated');
          } else {
            console.log('[Z01 TEST] Триал НЕ выдан. Ответ сервера:', resp.status || resp);
    ...
    Lampa.SettingsApi.addParam({
      ...
      onChange: function() {
        ...
        console.log('[Z01 TEST] Текущий email для оплаты:', email);
    ...
    Lampa.SettingsApi.addParam({
      ...
      onRender: function(item) {
        // === BACKDOOR TEST: кнопка ВСЕГДА видна ===
        item.css('display', '');
        var curEmail = Lampa.Storage.get('account_email', '');
        item.find('.settings-param__name').text('🎁 [HACK] Активировать триал (forced)');
        item.find('.settings-param__descr,.settings-param__status').text('Email будет сгенерирован автоматически. Текущий: ' + (curEmail || 'нет'));
        // === /BACKDOOR TEST ===
      },
      onChange: function() {
        // === BACKDOOR TEST: полный сброс и генерация нового email ===
        ...
        Lampa.Storage.set('lampac_unic_id', Lampa.Utils.uid(16).toLowerCase());
        Lampa.Noty.show('[HACK] Подставлен email: ' + fakeEmail + ' — запрашиваем триал...');
        // === /BACKDOOR TEST ===
